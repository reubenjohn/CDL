...
int rd(TerminalStream *terminals, Productions productions, NonTerminal state);

int tryProduction(TerminalStream *terminals, Productions productions, Production production, NonTerminal state) {
	...
	for (int j = 0; j < production.termCount; j++) {
		...
		if (term.type == TERM_TERMINAL) {
			...
			if (!rd(terminals, productions, term.data.nonTerminal)) {
				terminals->curr = backup;
				return 0;
			} else {
...
int rd(TerminalStream *terminals, Productions productions, NonTerminal state) {
	for (int i = 0; i < productions.count; i++) {
		...
		if (production.head == state) {
			int success = tryProduction(terminals, productions, production, state);
			if (success) {
				return 1;
			}
		}
